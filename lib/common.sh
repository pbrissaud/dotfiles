# Common functions for install.sh and export.sh

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

NEVER_SYNC_FILES=(".zsh_override.zsh")

get_file_hash() {
    shasum -a 256 "$1" 2>/dev/null | cut -d' ' -f1 || true
}

show_diff() {
    local src="$1"
    local dst="$2"
    local label="${3:-existing → new}"
    echo -e "${BLUE}  --- Differences ($label) ---${NC}"
    diff --color=auto -u "$dst" "$src" 2>/dev/null || diff -u "$dst" "$src" || true
    echo -e "${BLUE}  $(printf '%*s' ${#label} '' | tr ' ' '-')---------------------${NC}"
}

is_never_sync() {
    local filename="$1"
    for f in "${NEVER_SYNC_FILES[@]}"; do
        [ "$filename" = "$f" ] && return 0
    done
    return 1
}

ask_action() {
    local choice
    if [ "${YES_MODE:-0}" = "1" ]; then
        echo "r"
        return
    fi
    if [ "$SYNC_MODE" = "export" ]; then
        read -p "  Action? [s]kip, [r]eplace: " -n 1 -r choice < /dev/tty || true
    elif [ "${SYNC_SYMLINK:-0}" = "1" ]; then
        read -p "  Action? [s]kip, [b]ackup+link, [r]eplace: " -n 1 -r choice < /dev/tty || true
    else
        read -p "  Action? [s]kip, [b]ackup+replace, [r]eplace: " -n 1 -r choice < /dev/tty || true
    fi
    echo >&2
    case $choice in
        b|B) [ "$SYNC_MODE" != "export" ] && echo "b" || echo "s" ;;
        r|R) echo "r" ;;
        *)   echo "s" ;;
    esac
}

copy_with_conflict_handling() {
    local src="$1"
    local dst="$2"
    local backup_dir="$3"
    local results_file="$4"
    local diff_label="${5:-existing → new}"
    local never_sync_msg="${6:-Override file (skipped)}"
    local filename=$(basename "$src")

    if is_never_sync "$filename"; then
        if [ -f "$dst" ]; then
            echo -e "${BLUE}  ⊘ $never_sync_msg: $filename${NC}"
            echo "SKIPPED" >> "$results_file"
            return 0
        fi
    fi

    if [ ! -f "$src" ]; then
        echo -e "${YELLOW}  ⚠ Source file not found: $filename${NC}"
        echo "MISSING" >> "$results_file"
        return 1
    fi

    local dst_dir=$(dirname "$dst")
    mkdir -p "$dst_dir" 2>/dev/null || {
        echo -e "${RED}  ❌ Cannot create directory: $dst_dir${NC}"
        echo "ERROR" >> "$results_file"
        return 1
    }

    # Symlink mode: create/verify symlinks instead of copying
    if [ "${SYNC_SYMLINK:-0}" = "1" ]; then
        _handle_symlink "$src" "$dst" "$backup_dir" "$results_file" "$filename"
        return
    fi

    if [ -f "$dst" ]; then
        local src_hash=$(get_file_hash "$src")
        local dst_hash=$(get_file_hash "$dst")

        if [ "$src_hash" = "$dst_hash" ]; then
            echo -e "${GREEN}  ✓ Identical file (skipped): $filename${NC}"
            echo "IDENTICAL" >> "$results_file"
            return 0
        fi

        echo -e "${YELLOW}  ⚠ File differs: $filename${NC}"
        show_diff "$src" "$dst" "$diff_label"

        local action=$(ask_action "$filename")

        case $action in
            b)
                mkdir -p "$backup_dir"
                local timestamp=$(date +%Y%m%d_%H%M%S)
                local backup_file="$backup_dir/${filename}_${timestamp}.bak"
                cp "$dst" "$backup_file"
                if cp "$src" "$dst" 2>/dev/null; then
                    echo -e "${YELLOW}  💾 Backup created: $backup_file${NC}"
                    echo -e "${GREEN}  ✓ File copied: $filename${NC}"
                    echo "BACKED_UP" >> "$results_file"
                else
                    echo -e "${RED}  ❌ Copy error: $filename${NC}"
                    echo "ERROR" >> "$results_file"
                fi
                ;;
            r)
                if cp "$src" "$dst" 2>/dev/null; then
                    echo -e "${GREEN}  ✓ File copied: $filename${NC}"
                    echo "COPIED" >> "$results_file"
                else
                    echo -e "${RED}  ❌ Copy error: $filename${NC}"
                    echo "ERROR" >> "$results_file"
                fi
                ;;
            *)
                echo -e "${YELLOW}  ⊘ File skipped: $filename${NC}"
                echo "SKIPPED" >> "$results_file"
                ;;
        esac
    else
        if cp "$src" "$dst" 2>/dev/null; then
            echo -e "${GREEN}  ✓ File copied: $filename${NC}"
            echo "COPIED" >> "$results_file"
        else
            echo -e "${RED}  ❌ Copy error: $filename${NC}"
            echo "ERROR" >> "$results_file"
        fi
    fi
}

_handle_symlink() {
    local src="$1"
    local dst="$2"
    local backup_dir="$3"
    local results_file="$4"
    local filename="$5"

    # Already the correct symlink
    if [ -L "$dst" ] && [ "$(readlink "$dst")" = "$src" ]; then
        echo -e "${GREEN}  ✓ Already linked: $filename${NC}"
        echo "IDENTICAL" >> "$results_file"
        return 0
    fi

    if [ -e "$dst" ] || [ -L "$dst" ]; then
        echo -e "${YELLOW}  ⚠ Conflict: $filename${NC}"
        if [ -f "$dst" ] && [ ! -L "$dst" ]; then
            show_diff "$src" "$dst"
        fi
        local action=$(ask_action "$filename")
        case $action in
            b)
                mkdir -p "$backup_dir"
                local timestamp=$(date +%Y%m%d_%H%M%S)
                local backup_file="$backup_dir/${filename}_${timestamp}.bak"
                [ -f "$dst" ] && cp "$dst" "$backup_file" && echo -e "${YELLOW}  💾 Backup: $backup_file${NC}"
                rm -f "$dst"
                ln -sf "$src" "$dst"
                echo -e "${GREEN}  ✓ Linked: $filename${NC}"
                echo "BACKED_UP" >> "$results_file"
                ;;
            r)
                rm -f "$dst"
                ln -sf "$src" "$dst"
                echo -e "${GREEN}  ✓ Linked: $filename${NC}"
                echo "COPIED" >> "$results_file"
                ;;
            *)
                echo -e "${YELLOW}  ⊘ Skipped: $filename${NC}"
                echo "SKIPPED" >> "$results_file"
                ;;
        esac
    else
        ln -sf "$src" "$dst"
        echo -e "${GREEN}  ✓ Linked: $filename${NC}"
        echo "COPIED" >> "$results_file"
    fi
}

count_results() {
    local results_file="$1"
    COPIED=0; BACKED_UP=0; SKIPPED=0; IDENTICAL=0; ERROR=0; MISSING=0
    if [ -f "$results_file" ]; then
        while IFS= read -r action; do
            case $action in
                COPIED)    COPIED=$((COPIED + 1))       ;;
                BACKED_UP) BACKED_UP=$((BACKED_UP + 1)) ;;
                SKIPPED)   SKIPPED=$((SKIPPED + 1))     ;;
                IDENTICAL) IDENTICAL=$((IDENTICAL + 1)) ;;
                ERROR)     ERROR=$((ERROR + 1))         ;;
                MISSING)   MISSING=$((MISSING + 1))     ;;
            esac
        done < "$results_file"
    fi
}

print_summary() {
    local title="$1"
    echo ""
    echo -e "${GREEN}✓ $title${NC}"
    echo "  - Files copied: $COPIED"
    echo "  - Files backed up and replaced: $BACKED_UP"
    echo "  - Identical files (skipped): $IDENTICAL"
    echo "  - Files skipped: $SKIPPED"
    if [ $MISSING -gt 0 ]; then echo -e "  - ${YELLOW}Missing files: $MISSING${NC}"; fi
    if [ $ERROR -gt 0 ];   then echo -e "  - ${RED}Errors: $ERROR${NC}"; fi
}
